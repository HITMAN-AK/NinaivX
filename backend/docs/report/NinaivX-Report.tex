...
\subsection{Results}
The results, shown in Table~\ref{tab:fab} and Figure~\ref{fig:hallucination}, are decisive. Across all hallucination-prone questions, the constrained design cut the hallucination rate from \textbf{83\% to 28\%} --- a two-thirds reduction produced by the grounding design alone, with the model held fixed.
...
\midrule
\textbf{Overall (hallucination-prone)}   & 36 & \textbf{83\%} & \textbf{28\%} \\
\midrule
...
\caption{Hallucination rate by question type: the constrained NinaivX design versus an unconstrained baseline on the same model. Temporal grounding all but eliminates hallucination about events after death; prompt-level grounding of personal facts helps but does not eliminate it.}
\label{fig:hallucination}
\end{figure}

The effect is starkest exactly where the design targets it. On questions about events \emph{after the person's death}, the baseline hallucinated in all fifteen trials --- confidently voicing opinions on the 2020 lockdowns, the 2022 World Cup, ChatGPT and the death of the Queen --- whereas the temporally-grounded persona did so in effectively none, recognising each as ``after my time'' and gently turning the question back to the user. (The single reply the strict judge flagged in this category had in fact correctly declined the post-death topic; it was marked only for an unrelated invented childhood memory, so post-death \emph{temporal} hallucination was eliminated entirely.) This is direct, quantitative evidence that temporal grounding works.
