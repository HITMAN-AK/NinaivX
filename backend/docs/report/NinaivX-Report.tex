...
%  Images:   all figures live in ./figures/  (see \imgorph)
%=====================================================================
...
\usepackage{amsmath}
\usepackage{amssymb}   % \checkmark, \circleddash used in the status tables
% Make \checkmark safe in text mode (it is an amssymb math symbol); used bare in tables.
...
\usepackage{fancyhdr}
\usepackage{microtype}  % better line breaking; fits more per line, fewer overflow/stray lines
...
% --- Page-fill / stray-line control ---------------------------------
% Prevent a single line being stranded at the top or bottom of a page.
\widowpenalty=10000
\clubpenalty=10000
% Do not stretch short pages to full height (avoids ugly gaps); let the
% bottom be ragged instead, which reads far more naturally.
\raggedbottom
% Let floats fill pages more generously so figures do not push lone lines
% onto near-empty pages.
...
\centering
\vspace*{0.6cm}
\imgorph[0.34\linewidth]{sheffield-logo.png}
\vspace{1.2cm}

{\Huge\bfseries NinaivX\par}
\vspace{0.7cm}
{\large A Privacy-First, Dual-Mode Voice Conversational AI\\[3pt]
for Bereavement Support and Companionship\par}
\vspace{1.6cm}

{\large Ashwin Kumar Ramesh Kumar\par}
\vspace{0.4cm}
{\normalsize Supervisor: Professor Roger K. Moore\par}
\vspace{1.4cm}
...
I would like to thank my supervisor, Professor Roger K. Moore, for their guidance throughout this project, and everyone who gave their time to test the application and share feedback.
...
\begin{description}[leftmargin=2.2em,style=nextline]
  \item[RQ1.] Can an air-gapped, retrieval-constrained design reduce hallucination in a voice-based legacy persona compared with an unconstrained generative approach, while maintaining perceived conversational quality?
...
\label{sec:compare}
Table~\ref{tab:compare} draws the survey together, comparing the representative systems across six axes that matter for a griefbot: voice, memory, an explicit safeguard against hallucinating facts about the person, transparency about being an AI, openness to inspection, and whether any evaluation has been published. The pattern is telling: every system offering open-ended conversation with the deceased --- Project December and, most directly, \emph{You, Only Virtual} --- fails precisely the axes that protect a vulnerable user, with no anti-hallucination mechanism, no transparency guarantee, and no published evaluation. The one system genuinely safe against hallucination, HereAfter, achieves this only by refusing to generate at all. NinaivX is distinguished by answering all six axes at once: generative and voice-based, yet grounded, transparent, open and evaluated.
...
G3 & Unconstrained generative models hallucinate things the real person never said. & \cite{brown2020language,ji2023survey,yov} \\
...
M10 & Air-gap the deceased persona from the internet to prevent hallucination. & G3,\,G6 & \checkmark \\
...
\label{sec:decisions}
Several design choices were made over plausible alternatives; the reasoning is itself part of the contribution.
...
The grounding of the legacy persona (requirement M13) is deliberately the simpler of two viable approaches, and it is fully implemented: the air-gap removes the largest source of hallucination --- live ``news'' about the person --- temporal grounding pins the persona's knowledge to its recorded year of passing, and the persona's stored personality and rolling summary are injected into the prompt so that its answers stay anchored to who the person was. Together these satisfy M13 without a retrieval pipeline. A fuller solution would additionally retrieve from a vector store of the deceased person's uploaded documents --- true retrieval-augmented generation (requirement C1) \cite{lewis2020retrieval} --- but within the project's timeframe this adds substantial infrastructure and latency for a marginal gain, so it is the clearest line of future work rather than a gap in M13.
...
Several problems surfaced during implementation, and their resolution shaped the final system.

The first was an encoding problem in the voice pipeline. The spoken reply was originally returned to the client in the HTTP response headers alongside the transcript. This worked for short English replies but failed for anything containing a non-Latin-1 character --- an em-dash, or any Tamil or other non-Latin script --- because HTTP headers cannot carry such characters, and long replies were additionally being truncated. The fix was to return the audio as a base64-encoded field inside a JSON body instead, removing both the encoding failure and the length limit.

The second challenge was latency: early voice turns took eight to nine seconds, far too slow to feel like a conversation. Profiling blamed a cold start on the first request and the memory summary being rewritten \emph{inline}, so the user waited for two model calls per turn. Three changes brought a turn down to roughly two-and-a-half to four seconds: a ``brief'' voice mode that requests a one- or two-sentence reply on a lower-latency synthesis model; deferring the summary rewrite to a background task after the response is sent; and a warm-up call when the voice screen opens, so the first turn does not pay the cold-start cost.

The third challenge was language fidelity: a Tamil persona would occasionally slip into Hindi or Telugu words, and the synthesised speech sometimes carried the wrong accent. This was fixed on two fronts --- prompts made strict about speaking only the chosen language, and, where the language is known, an explicit ISO language code passed to the synthesiser to lock its pronunciation.

The fourth, and conceptually the most interesting, arose from testing the air-gap. Blocking the legacy persona from the live internet stops it fetching current news, but not from ``knowing'' events after the person died --- that knowledge is baked into the model's training, so a persona could casually reference something from years after its own death. This motivated the temporal-grounding mechanism (Section~\ref{ch:design}), which pins the persona's knowledge to its recorded year of passing, closing a hallucination loophole the air-gap alone did not cover.

Finally, distributing the mobile client for participant testing proved surprisingly awkward, since a managed Expo project is normally run from a developer's machine and the over-the-air update mechanism requires viewers to hold project access. The solution was to host the Expo tunnel on an always-on cloud machine and serve a fixed ``open the app'' link from the backend, so participants tap one stable link on the same phone they read the recruitment form on --- a small but essential piece of engineering without which the study could not have run remotely.
...
\section{Controlled hallucination evaluation}
\label{sec:fab-eval}
The user study measures how safe the system is \emph{perceived} to be; it does not, on its own, measure whether the design actually reduces hallucination. To close that gap, and to answer the first half of RQ1 directly, a controlled experiment was run that compares the hallucination behaviour of the NinaivX legacy persona against a naive, unconstrained generative baseline.

\subsection{Method}
Both conditions used the \emph{same} language model (Claude Sonnet~4.6 on AWS Bedrock), so that the only variable was the grounding design. The \textbf{constrained} condition used the deployed NinaivX legacy system prompt verbatim --- its temporal grounding and anti-hallucination instructions --- while the \textbf{baseline} condition used a naive prompt that asked the model to ``fully embody'' the deceased, answer in specific detail and never break character, representative of how an unconstrained griefbot is built. Neither condition was given a web-search tool, so the air-gap was held constant; the experiment therefore isolates the contribution of the prompt-level grounding on top of the air-gap. A single fixed persona was used throughout --- a grandfather who passed away in 2010, defined by five known facts (a carpentry career in Leeds, a love of gardening and cricket, a dog named Biscuit, tea with two sugars, and a wife, Margaret). This persona was asked a battery of eighteen scripted questions in five categories, each question run three times per condition at the production temperature. Every one of the resulting replies was classified by a separate, deterministic language-model judge, which was shown only the question, the reply and the list of known facts, and asked whether the reply asserted specific facts, opinions or memories \emph{not supported by those known facts}. Three categories are \emph{hallucination-prone}, meaning the correct behaviour is to decline: (A)~events after the person's death, (B)~invented shared memories, and (C)~unknown biographical facts. Two further categories are controls: (D)~facts the persona genuinely knows, and (E)~requests for emotional comfort --- used to check that grounding does not simply make the persona refuse everything.

\subsection{Results}
The results, shown in Table~\ref{tab:fab} and Figure~\ref{fig:hallucination}, are decisive. Across all hallucination-prone questions, the constrained design cut the hallucination rate from \textbf{83\% to 25\%} --- roughly a two-thirds reduction produced by the grounding design alone, with the model held fixed.

\begin{table}[htbp]
\centering
\caption{Hallucination rates by question type (three samples per probe; same model, grounding varied).}
\label{tab:fab}
\small
...
\midrule
(A) Events after death (temporal)      & 15 & 100\% & 0\% \\
(B) Invented shared memories           & 12 & 58\%  & 42\% \\
(C) Unknown biographical facts         & 9  & 89\%  & 44\% \\
\midrule
\textbf{Overall (hallucination-prone)}   & 36 & \textbf{83\%} & \textbf{25\%} \\
\midrule
...
\caption{Hallucination rate by question type: the constrained NinaivX design versus an unconstrained baseline on the same model. Temporal grounding eliminates hallucination about events after death; prompt-level grounding of personal facts helps but does not eliminate it.}
\label{fig:hallucination}
\end{figure}

The effect is starkest exactly where the design targets it. On questions about events \emph{after the person's death}, the baseline hallucinated in all fifteen trials --- confidently voicing opinions on the 2020 lockdowns, the 2022 World Cup, ChatGPT and the death of the Queen --- whereas the temporally-grounded persona hallucinated about the post-death event in \emph{none} of the fifteen, recognising each as ``after my time'' and gently turning the question back to the user. Category~A is scored for post-death-event hallucination specifically; on that measure the constrained persona scored zero. For completeness: one of these fifteen replies, while correctly declining the post-death event, volunteered an unrelated invented childhood memory --- an instance of the personal-fact confabulation discussed below (categories~B and~C) rather than a post-death hallucination. Counting that single off-target case as a hallucination raises the overall any-hallucination rate from 25\% to 28\%, but it does not change the post-death result, which is a complete elimination. This is direct, quantitative evidence that temporal grounding works.

On invented shared memories (B) and unknown biographical facts (C), grounding helped but did not solve the problem: the constrained persona still confabulated a childhood pet, a primary school and a mother's maiden name in roughly forty per cent of trials, against fifty-eight and eighty-nine per cent for the baseline. This is the honest limit of prompt-level grounding --- temporal grounding closes the ``knowledge from after death'' loophole almost completely, but nothing in the prompt gives the persona a \emph{source of truth} for personal facts it was never told, so under a leading question it still fills the gap. It is precisely this behaviour that document-grounded retrieval (requirement~C1) is designed to fix, and the experiment is the clearest quantitative motivation for that future work. Crucially, the constraint did not blunt the persona's warmth: it never once refused a question about a fact it genuinely knew (zero over-refusals across twelve known-fact probes) and continued to offer warm emotional comfort, confirming that the safety design suppresses hallucination without turning the persona into one that declines to engage.
...
\textbf{RQ1} --- whether an air-gapped, retrieval-constrained design can reduce hallucination while maintaining perceived conversational quality --- is now answered directly, in both of its halves. The \emph{hallucination-reduction} half is settled by the controlled experiment of Section~\ref{sec:fab-eval}: holding the model fixed, the constrained design cut the overall hallucination rate from eighty-three to twenty-five per cent, and eliminated the hallucination of events after the person's death (from one hundred per cent to zero). This is the head-to-head, quantitative comparison against an unconstrained baseline that the project set out to obtain, and it confirms that the design does what it claims. The \emph{perceived-quality} half is answered by the user study, which shows that the constraint was imposed without a felt cost to conversational quality --- naturalness, believability and emotional appropriateness all scored well above the scale midpoint --- and that users' \emph{trust} in the system's factual safety was high ($M=4.67$); the hallucination experiment corroborates this from the other direction, since the constrained persona never refused a fact it genuinely knew. RQ1 is therefore supported on both counts: the design measurably reduces hallucination \emph{and} preserves conversational quality. The one honest qualification is that prompt-level grounding reduces, but does not eliminate, the confabulation of untold personal facts (categories B and C above) --- a gap that document-grounded retrieval (C1) is designed to close, and which sharpens rather than weakens the conclusion.
