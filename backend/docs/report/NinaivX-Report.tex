...
% TODO IMAGE: replace with the University of Sheffield crest (logo).
\placeholder{University of Sheffield logo}{}
\vspace{1.5cm}
...
% TODO IMAGE: a simple diagram of the two agent paths.
\placeholder{Agent graph: supervisor $\rightarrow$ \{deceased node $\rightarrow$ END\} vs \{companion node $\leftrightarrow$ tools\}. Draw a simple two-branch diagram.}{The agent graph, showing the air-gapped deceased path and the web-connected companion loop.}{fig:graph}
...
\placeholder{Entity-relationship diagram: User (1)---&lt;(many) Persona (1)---(1) Conversation, with key fields listed.}{The NinaivX data model.}{fig:erd}
...
% TODO IMAGE: add 2–4 app screenshots (persona list, create wizard, chat, voice call).
\placeholder{App screenshots: persona list, the create-persona wizard, the text chat, and the voice-call screen.}{Key screens of the NinaivX mobile application.}{fig:screens}
...
% TODO IMAGE: use your existing docs/NinaivX-Workflow.png
\placeholder{End-to-end workflow (install $\rightarrow$ sign up $\rightarrow$ create persona $\rightarrow$ converse). Use docs/NinaivX-Workflow.png}{The end-to-end workflow of NinaivX.}{fig:workflow}
